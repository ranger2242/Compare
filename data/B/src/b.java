...
import com.quadx.dungeons.shapes1_5_1.Circle;
import com.quadx.dungeons.shapes1_5_1.Line;
import com.quadx.dungeons.shapes1_5_1.ShapeRendererExt;
import com.quadx.dungeons.shapes1_5_1.Triangle;
...
import com.quadx.dungeons.tools.timers1_0.Delta;
import com.quadx.dungeons.tools.timers1_0.Timer;
...
import static com.quadx.dungeons.tools.timers1_0.Time.SECOND;
import static com.quadx.dungeons.tools.timers1_0.Time.ft;
...
public class Player extends Drawable {
    ...
    public void setAimVector(Vector2 end, boolean overrideMouse) {
        ...
        if (!overrideMouse) {
            start = scr.cpy().scl(.5f);
        }
        body.setFacing(Direction.of(start.cpy().sub(end).nor()));
    }
...
//            lvlupEffect.start();
    ...
    public void updateMapState(float dt) {
        ...
        updatePotions(dt);
        getAttack().updateSub(Gdx.graphics.getDeltaTime());
        swim(dt);
        dig();
...
/////////////////////////////////////////////////
    ...
    public void renderSRLines(ShapeRendererExt sr) {
        sr.setColor(WHITE);
        for (Line l : getAttackChain()) {
            sr.line(l);
        }
    }

    public void notifyKill(MonsterStat st) {
        int lvl = st.getLevel();
        out(st.getName() + " Level " + lvl + " was killed.");
        addKills();
        genExp(lvl, st.getExpFactor());
        checkLvlUp();
        addGold(new Gold(lvl));

    }
    public void genExp(int lvl, float factor) {
        double a = 50.9055;
        double b = 1.0015;
